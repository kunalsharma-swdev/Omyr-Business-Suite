...
export default function ProductDetail() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen pt-28 pb-20 bg-[#FFF8FC]">
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen pt-32 pb-20 bg-[#FFF8FC] flex items-center justify-center">
        <div className="text-center p-12 bg-white border border-[#E5E7EB] rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.06)] max-w-lg mx-auto">
          <div className="w-16 h-16 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-5">
            <AlertCircle className="w-8 h-8 text-red-400" />
          </div>
          <h2 className="text-2xl font-serif font-light text-[#1F2937] mb-2">Product Not Found</h2>
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 bg-[#FFF8FC]">
      <div className="container mx-auto px-4 md:px-6 max-w-6xl">

        {/* Breadcrumbs */}
        <nav className="flex items-center text-xs font-medium text-[#6B7280] mb-8 overflow-x-auto whitespace-nowrap pb-1 gap-1.5 font-sans">
          ...
          <ChevronRight className="w-3.5 h-3.5 text-[#E5E7EB]" />
          <span className="text-[#1F2937] font-medium truncate max-w-[200px]">{formatName(product.product_name)}</span>
        </nav>

        <div className="bg-white rounded-3xl shadow-[0_8px_40px_rgba(0,0,0,0.08)] border border-[#E5E7EB]/60 overflow-hidden">
          <div className="flex flex-col lg:flex-row">

            {/* Image */}
            <div className="w-full lg:w-[48%] relative bg-gradient-to-br from-[#FFF8FC] to-[#F3E8FF] flex items-center justify-center min-h-[400px] lg:min-h-[600px]">
              {product.image_name ? (
                <img
              ...
              <div className="absolute top-4 left-4">
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/95 backdrop-blur-sm text-[#E8177A] font-sans text-[10px] font-bold tracking-[0.18em] uppercase rounded-full shadow-sm">
            ...
            <div className="w-full lg:w-[52%] p-8 md:p-12 flex flex-col">
              ...
              <h1 className="text-3xl md:text-4xl font-serif font-light text-[#1F2937] mb-5 leading-tight">
              ...
              {product.description && (
                <p className="text-[#6B7280] leading-relaxed font-sans text-base mb-8">
                  {product.description}
                </p>
              )}

              {/* Rental notice */}
              <div className="bg-gradient-to-r from-[#FFF8FC] to-[#F3E8FF] border border-[#E8177A]/15 rounded-2xl p-5 mb-8">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-sans font-semibold text-[#1F2937] text-sm mb-1">Available for Rental</h4>
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                {["Premium Quality", "Sanitized", "All Sizes"].map((b) => (
                  <div key={b} className="text-center p-3 bg-[#FFF8FC] rounded-xl border border-[#E5E7EB]/80">
                    <p className="text-[#1F2937] font-sans text-xs font-semibold">{b}</p>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-8 text-center">
          <Link href={`/catalogue/${encodeURIComponent(product.category)}`}>
            <span className="inline-flex items-center gap-2 text-[#6B7280] hover:text-[#E8177A] font-sans text-sm font-medium transition-colors duration-200 cursor-pointer group">
