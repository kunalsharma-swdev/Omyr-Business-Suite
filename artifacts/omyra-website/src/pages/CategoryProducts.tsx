...
export default function CategoryProducts() {
  ...
  return (
    <div className="min-h-screen bg-[#FFF8FC]">
      ...
      {/* Header */}
      <div className="relative pt-24 pb-12 bg-gradient-to-br from-white via-[#FFF8FC] to-[#FDF4FF] border-b border-[#E5E7EB]">
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-7xl relative z-10">
          <Link
            href="/catalogue"
            className="inline-flex items-center gap-2 text-[#6B7280] hover:text-[#E8177A] font-sans text-sm font-medium transition-colors duration-200 mb-6 group"
          >
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              ...
              <h1 className="text-4xl md:text-5xl font-serif font-light text-[#1F2937] capitalize">
                {formatName(decodedCategory)}
              </h1>
              {!isLoading && data && (
                <p className="text-[#6B7280] mt-2 font-sans text-sm">
                  {data.products.length} of {data.totalCount} costumes available for rent
                </p>
              )}
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-[#E8177A]/8 rounded-full border border-[#E8177A]/20">
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-7xl py-10">
        {isLoading ? (
          ...
          <>
            ...
            {/* Pagination */}
            {data.totalPages > 1 && (
              <div className="flex items-center justify-center gap-4 mt-14 pt-8 border-t border-[#E5E7EB]">
                <button
                  onClick={() => setPage((p) => Math.max(1, p - 1))}
                  disabled={page === 1}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-full border border-[#E5E7EB] text-[#6B7280] font-sans text-sm font-medium hover:border-[#E8177A] hover:text-[#E8177A] disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
                >
                ...
                <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className={`w-9 h-9 rounded-full font-sans text-sm font-medium transition-all duration-200 ${
                          page === p
                            ...
                            : "text-[#6B7280] hover:bg-[#FFF8FC] hover:text-[#E8177A]"
                        }`}
                      >
                        {p}
                      </button>
                  ...
                  {data.totalPages > 7 && (
                    <span className="text-[#6B7280] font-sans text-sm px-1">…{data.totalPages}</span>
                  )}
                </div>
          ...
          <div className="text-center py-24 bg-white rounded-2xl border border-[#E5E7EB] max-w-2xl mx-auto shadow-[0_4px_20px_rgba(0,0,0,0.05)]">
            <ShoppingBag className="w-12 h-12 text-[#E5E7EB] mx-auto mb-4" />
            <h2 className="text-2xl font-serif font-light text-[#1F2937] mb-3">No products found</h2>
