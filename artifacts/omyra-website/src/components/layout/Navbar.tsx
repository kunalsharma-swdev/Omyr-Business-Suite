    ...
    <>
      <nav
        className={`fixed top-0 w-full z-50 transition-all duration-300 ${
          scrolled
            ? "bg-white/98 dark:bg-[#0F0C1E]/98 backdrop-blur-xl shadow-[0_1px_20px_rgba(0,0,0,0.08)] dark:shadow-[0_1px_20px_rgba(0,0,0,0.3)] py-2.5"
            : "bg-white/95 dark:bg-[#0F0C1E]/95 backdrop-blur-md border-b border-[#E5E7EB] dark:border-[#23203A] py-3.5"
        }`}
      >
          ...
          <div className="flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2.5 z-50 group">
              <img
                src={getLogoUrl()}
                alt="Omyra Logo"
                className="h-10 w-auto object-contain transition-transform duration-300 group-hover:scale-105 dark:brightness-0 dark:invert dark:opacity-90"
                onError={(e) => {
              ...
              />
              <span className="logo-text hidden font-serif text-2xl font-light text-[#1F2937] dark:text-[#F1F0F5]">
                Omyra
              </span>
            </Link>
                  ...
                  <li key={link.name}>
                    <Link
                      ...
                      className={`font-sans text-sm font-medium tracking-wide transition-all duration-200 relative group py-1 ${
                        location === link.href
                          ? "text-[#E8177A]"
                          : "text-[#6B7280] dark:text-[#8B8499] hover:text-[#1F2937] dark:hover:text-[#F1F0F5]"
                      }`}
                    >
                      {link.name}
                      <span
            ...
            <div className="flex items-center gap-2 md:hidden">
              ...
              <button
                className="z-50 w-9 h-9 flex items-center justify-center rounded-full hover:bg-[#FFF8FC] dark:hover:bg-[#16112A] transition-colors text-[#1F2937] dark:text-[#F1F0F5]"
      ...
      >
        ...
        />
        <div
          className={`absolute top-0 right-0 h-full w-[280px] bg-white dark:bg-[#0F0C1E] shadow-2xl transform transition-transform duration-300 ease-out ${
                ...
                <li key={link.name}>
                  <Link
                    ...
                    className={`block py-3.5 px-4 rounded-xl font-sans text-lg font-light transition-colors duration-200 ${
                      location === link.href
                        ? "text-[#E8177A] bg-[#FFF8FC] dark:bg-[#16112A]"
                        : "text-[#1F2937] dark:text-[#F1F0F5] hover:bg-[#FFF8FC] dark:hover:bg-[#16112A]"
                    }`}
                  >
                    {link.name}
                  </Link>
